...
theorem fnLb_add (hfa : FnLb f a) (hgb : FnLb g b) : FnLb (fun x ↦ f x + g x) (a + b) := fun x ↦ add_le_add (hfa x) (hgb x)
...
theorem fnUb_mul (hfa : FnUb f a) (h : c ≥ 0) : FnUb (fun x ↦ c * f x) (c * a) := fun x ↦ mul_le_mul_of_nonneg_left (hfa x) h
...
example {c : ℝ} (h : c ≠ 0) : Surjective fun x ↦ c * x := by
  ...
  use x / c
  -- ring_nf
  dsimp; apply mul_div_cancel₀; exact h

example (x y : ℝ) (h : x - y ≠ 0) : (x ^ 2 - y ^ 2) / (x - y) = x + y := by
  -- goal: (x ^ 2 - y ^ 2) / (x - y) = x + y
  field_simp [h]
  -- goal: x ^ 2 - y ^ 2 = (x + y) * (x - y)
  ring
...
example (surjg : Surjective g) (surjf : Surjective f) : Surjective fun x ↦ g (f x) := by
  intro y
  rcases surjg y with ⟨a, gax, rfl⟩
  rcases surjf a with ⟨b, fbx, rfl⟩
  dsimp; use b
