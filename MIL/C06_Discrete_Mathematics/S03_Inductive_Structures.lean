...
theorem depth_le_size : ∀ t : BinTree, depth t ≤ size t
  ...
  | node l r => by
      simp [depth, size]
      constructor <;> linarith [depth_le_size l, depth_le_size r]
...
theorem size_flip : ∀ t, size (flip t) = size t := by
  ...
  · simp [flip]
  · simp [flip, size, ihl, ihr, add_comm]
end BinTree
...
theorem subst_eq_of_not_mem_vars :
    ...
    | var m,    _, _, _ => by simp_all [subst, vars]; intro h'; symm at h'; contradiction
    | fls,      _, _, _ => by simp [subst]
    | conj _ _, _, _, _ => by simp_all [subst, vars, subst_eq_of_not_mem_vars]
    | disj _ _, _, _, _ => by simp_all [subst, vars, subst_eq_of_not_mem_vars]
    | impl _ _, _, _, _ => by simp_all [subst, vars, subst_eq_of_not_mem_vars]
...
theorem subst_eval_eq : ∀ (A : PropForm) (n : ℕ) (C : PropForm) (v : ℕ → Bool),
  ...
  | var _, _, _, _ => by
      simp [subst, eval]
      split <;> rfl
  | fls, _, _, _ => by simp [subst, eval]
  | conj _ _, _, _, _ => by simp [subst, eval, subst_eval_eq]
  | disj _ _, _, _, _ => by simp [subst, eval, subst_eval_eq]
  | impl _ _, _, _, _ => by simp [subst, eval, subst_eval_eq]
